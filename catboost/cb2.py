...
for k, (train_index, test_index) in enumerate(kf.split(train_X)):
    ...
    cb_model = CatBoostClassifier(
        eval_metric='AUC',
        depth=10,
        iterations=500+250*k,
        learning_rate=0.01,
        rsm=0.1
    )
    # [1+1, 2+1, 3+1, 4+1, 15+1, 21+1, 25+1]
    cf = [list(train_data.columns).index(x) for x in ['V2', 'V3', 'V4', 'V5', 'V16', 'V22', 'V26']]
    cb_model.fit(train_data, train_label, cat_features=cf)
    test_pred = cb_model.predict_proba(test_data)
    score = roc_auc_score(test_label, test_pred[:, 1])
    print('k =', k, 'auc_score =', score)
    fname.write('iter =' + str(500+250*k) + 'num of 1 =' + str(sum(test_label)) + ' auc_score =' + str(score) + '\n')
